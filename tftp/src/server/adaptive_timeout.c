...
static inline struct timespec double_to_timespec(double d) {
    ...
    constexpr typeof(t->tv_sec) max_sec = (((unsigned long long) 1 << (sizeof(t->tv_sec) * CHAR_BIT - 1)) - 1);
    if (d > (double)max_sec) {
        d = (double)max_sec + 1.0 - 1e-9;
    }
    if (d < 1e-9) {
        d = 0.0;
    }
    struct timespec ts = {
        ...
        .tv_nsec = (typeof(ts.tv_nsec)) ((d - (typeof(ts.tv_sec)) d) * 1e9),
    };
...
void adaptive_timeout_init(struct adaptive_timeout at[static 1]) {
    *at = (struct adaptive_timeout) {
        ...
        .rto = {.tv_sec = 0, .tv_nsec = 125'000'000},
    };
}
