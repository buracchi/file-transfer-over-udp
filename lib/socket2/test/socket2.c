...
int client() {
	...
	try(socket = socket2_init(tproto, nproto), NULL, error);
...
int server() {
	...
	try(server = socket2_init(tproto, nproto), NULL, error);
	...
	try(socket2_destroy(client), 1, error);
	try(socket2_destroy(server), 1, error);
error:
	return EXIT_FAILURE;
}